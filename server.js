...
async function requestGhlApi(pathname, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxRetries + 1; attempt += 1) {
    ...
    const responseContentType = normalizeAttachmentMimeType(response.headers.get("content-type"));
    if (response.ok) {
      return {
        ok: true,
...
async function resolveGhlContractTextByCandidateId(candidateId, context = {}, debugTrace = null) {
  const normalizedCandidateId = extractLikelyGhlEntityId(candidateId);
  if (!normalizedCandidateId) {
    if (debugTrace && typeof debugTrace === "object") {
      debugTrace.error = "missing_candidate_id";
  ...
  if (normalizedContactId) {
    ...
    attempts.push(
      ...
      {
        source: "contract_text.contacts.documents.by_id.preview",
        request: () =>
          requestGhlApi(`/contacts/${encodedContactId}/documents/${encodedCandidateId}/preview`, {
            method: "GET",
            query: {
              locationId: GHL_LOCATION_ID,
            },
  ...
  attempts.push(
    ...
    },
    {
      ...
      request: () =>
        requestGhlApi("/proposals/document", {
          ...
          query: {
            locationId: GHL_LOCATION_ID,
            query: [normalizedCandidateId, normalizedContactName, normalizedClientName].filter(Boolean).join(" ").trim(),
            skip: 0,
            limit: GHL_PROPOSAL_DOCUMENT_QUERY_LIMIT,
          },
          tolerateNotFound: true,
    ...
    {
      source: "contract_text.proposals.documents.by_id.preview",
      request: () =>
        requestGhlApi(`/proposals/documents/${encodedCandidateId}/preview`, {
          method: "GET",
          query: {
            locationId: GHL_LOCATION_ID,
          },
    ...
    {
      source: "contract_text.proposals.document.preview_path",
      request: () =>
        requestGhlApi(`/proposals/document/preview/${encodedCandidateId}`, {
          method: "GET",
          query: {
            locationId: GHL_LOCATION_ID,
          },
    ...
    {
      source: "contract_text.proposals.documents.by_id.content",
      request: () =>
        requestGhlApi(`/proposals/documents/${encodedCandidateId}/content`, {
          method: "GET",
          query: {
            locationId: GHL_LOCATION_ID,
          },
    ...
    {
      source: "contract_text.proposals.document.content_path",
      request: () =>
        requestGhlApi(`/proposals/document/content/${encodedCandidateId}`, {
          method: "GET",
          query: {
            locationId: GHL_LOCATION_ID,
          },
    ...
    {
      source: "contract_text.proposals.documents.by_id.query_html",
      request: () =>
        requestGhlApi(`/proposals/documents/${encodedCandidateId}`, {
          method: "GET",
          query: {
  ...
  for (const attempt of attempts) {
    ...
    const rawText = sanitizeTextValue(response?.rawText, 600000);
    let rawTextExtraction = null;
    if (rawText) {
      let jsonFromRaw = null;
      if (!response?.body && /^[\s]*[\[{]/.test(rawText)) {
        try {
          jsonFromRaw = JSON.parse(rawText);
        } catch {
          jsonFromRaw = null;
        }
      }

      if (jsonFromRaw && typeof jsonFromRaw === "object") {
        rawTextExtraction = extractGhlContractTextFromPayload(jsonFromRaw, {
          candidateId: normalizedCandidateId,
        });
      } else if (shouldUseGhlContractTextValue(rawText, attempt.source)) {
        const normalizedRawText = normalizeGhlContractTextFragment(rawText);
        if (normalizedRawText) {
          rawTextExtraction = {
            text: normalizedRawText,
            fragments: [normalizedRawText],
            truncated: normalizedRawText.length > GHL_CLIENT_CONTRACT_TEXT_MAX_CHARS,
            sourceNodeCount: 0,
          };
        }
      }
    ...
    const bestExtraction = extraction.text
      ? extraction
      : rawTextExtraction && rawTextExtraction.text
        ? rawTextExtraction
        : null;
    if (debugTrace?.attempts instanceof Array) {
      debugTrace.attempts.push({
        ...
        contentType: sanitizeTextValue(response?.contentType, 120),
        textFound: Boolean(bestExtraction?.text),
        textLength: bestExtraction?.text ? bestExtraction.text.length : 0,
        fragmentsCount: Array.isArray(bestExtraction?.fragments) ? bestExtraction.fragments.length : 0,
        rawTextLength: rawText.length,
      });
    }

    if (!bestExtraction?.text) {
      continue;
    }
    ...
    return {
      ...
      textLength: bestExtraction.text.length,
      fragmentsCount: Array.isArray(bestExtraction.fragments) ? bestExtraction.fragments.length : 0,
      truncated: Boolean(bestExtraction.truncated),
      sourceNodeCount: Number.isFinite(bestExtraction.sourceNodeCount) ? bestExtraction.sourceNodeCount : 0,
    };
  }
...
app.get("/api/ghl/client-contracts", requireWebPermission(WEB_AUTH_PERMISSION_VIEW_CLIENT_MANAGERS), async (req, res) => {
  ...
  const debugMode = parseBooleanFlag(req.query.debug, false);
  if (debugMode && !requestedClientNameRaw) {
    res.status(400).json({
      error: "Query parameter `clientName` is required when `debug=1`.",
    });
    return;
  }
  ...
  try {
    ...
    const items = await buildGhlClientContractDownloadRows(clientNames, {
      ...
      fastMode: !requestedClientNameRaw && !debugMode,
    });
...
app.get("/api/ghl/client-contracts/text", requireWebPermission(WEB_AUTH_PERMISSION_VIEW_CLIENT_MANAGERS), async (req, res) => {
  if (
    !enforceRateLimit(req, res, {
      scope: "api.ghl.client_contracts.text",
      ipProfile: {
        windowMs: RATE_LIMIT_PROFILE_API_SYNC.windowMs,
        maxHits: RATE_LIMIT_PROFILE_API_SYNC.maxHitsIp,
        blockMs: RATE_LIMIT_PROFILE_API_SYNC.blockMs,
      ...
      message: "Contract text lookup limit reached. Please wait before retrying.",
      code: "ghl_client_contract_text_rate_limited",
    })
  ) {
    return;
  }
  ...
  const requestedClientName = sanitizeTextValue(req.query.clientName, 300);
  const requestedContactId = sanitizeTextValue(req.query.contactId, 160);
  const requestedCandidateId = extractLikelyGhlEntityId(req.query.candidateId);
  const debugMode = parseBooleanFlag(req.query.debug, false);
  if (!requestedClientName) {
    res.status(400).json({
      error: "Query parameter `clientName` is required.",
  ...
  try {
    ...
    res.json({
      ok: true,
      clientName: matchedClientName,
      contactName: sanitizeTextValue(resolvedText.contactName, 300) || sanitizeTextValue(lookupRow?.contactName, 300) || "-",
      contactId: sanitizeTextValue(resolvedText.contactId, 160) || sanitizeTextValue(lookupRow?.contactId, 160) || "",
      contractTitle: sanitizeTextValue(lookupRow?.contractTitle, 300) || "Contract",
      candidateId: sanitizeTextValue(resolvedText.candidateId, 220),
      source: sanitizeTextValue(resolvedText.source, 200),
      text: sanitizeTextValue(resolvedText.text, GHL_CLIENT_CONTRACT_TEXT_MAX_CHARS + 20),
      textLength: Number.isFinite(resolvedText.textLength) ? resolvedText.textLength : 0,
    ...
    console.error("GET /api/ghl/client-contracts/text failed:", error);
    res.status(error.httpStatus || 502).json({
      error: sanitizeTextValue(error?.message, 500) || "Failed to load contract text from GHL.",
    });
  }
});
